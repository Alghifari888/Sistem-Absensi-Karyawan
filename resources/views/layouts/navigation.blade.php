            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(auth()->user()->isKaryawan() || auth()->user()->isAtasan())
                        ...
                        <x-nav-link :href="auth()->user()->isKaryawan() ? route('karyawan.leaves.index') : route('atasan.leaves.index')" :active="request()->routeIs('karyawan.leaves.*') || request()->routeIs('atasan.leaves.*')">
                            {{ __('Cuti & Izin') }}
                        </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->isKaryawan() || auth()->user()->isAtasan())
                ...
                <x-responsive-nav-link :href="auth()->user()->isKaryawan() ? route('karyawan.leaves.index') : route('atasan.leaves.index')" :active="request()->routeIs('karyawan.leaves.*') || request()->routeIs('atasan.leaves.*')">
                    {{ __('Cuti & Izin') }}
                </x-responsive-nav-link>
            @endif
            ...
            @if(auth()->user()->isAdmin())
                <x-responsive-nav-link :href="route('admin.settings.index')" :active="request()->routeIs('admin.settings.index')">
                 ...
                 <x-responsive-nav-link :href="route('admin.qrcode.show')" :active="request()->routeIs('admin.qrcode.show')">
                    {{ __('QR Code Absensi') }}
                </x-responsive-nav-link>
            @endif
        </div>
